...
def set_pipewire_default(kind: str, name: str) -> None:
    try:
        subprocess.run(
            [
                "pw-metadata",
...
class WebRTCSession:
    ...
    def _on_incoming_stream(self, _webrtc, pad: Gst.Pad) -> None:
        caps = pad.get_current_caps()
        if self.microphone or not self.pipeline or not caps or caps.is_empty():
            return
